...
class MetadatiCustom(p.SingletonPlugin, tk.DefaultDatasetForm):
	...
	def get_helpers(self):
		...
		'user_list':user_list, 'allowed_list':allowed_list}
	
	def get_validators(self):
		return{'gemet_converter':gemet_converter,
		       'converter_from_ico_to_dcat':converter_from_ico_to_dcat,
		       'converter_load_rdf':converter_load_rdf
			}
		


	#################################
	# METADATI IN CREATE / UPDATE PKG
	#################################
	def _modify_package_schema(self, schema):
		...
		schema.update({
				...
				tk.get_converter('convert_to_tags')('autorizzati')]
		})	
		...
		schema.update({
			'ico_categoria_gemet': [
				tk.get_validator('ignore_missing'),
				tk.get_converter('convert_to_tags')('ico_categoria_gemet') 			
			]
		})
		schema.update({
			'ico_sottocategoria_gemet': [
				tk.get_validator('ignore_missing'),
				tk.get_converter('convert_to_tags')('ico_sottocategoria_gemet') 			
			]
		})
		...
		schema.update({
			'load_rdf_job_start': [tk.get_validator('ignore_missing'),
				tk.get_converter('converter_load_rdf')]
		})
	...
	def show_package_schema(self):
		...
		schema['tags']['__extras'].append(tk.get_converter('free_tags_only'))#### ICO CAT GEMET
		schema.update({
			'ico_categoria_gemet': [
		...
		schema['tags']['__extras'].append(tk.get_converter('free_tags_only'))######## ICO SOTTO GEMET
		schema.update({
			'ico_sottocategoria_gemet': [
				tk.get_converter('convert_from_tags')('ico_sottocategoria_gemet'),
				tk.get_validator('ignore_missing')]
			})
		
		schema['tags']['__extras'].append(tk.get_converter('free_tags_only'))########## DCAT THEME
		schema.update({
			'theme': [
		...
		schema.update({
				'autorizzati': [
					tk.get_converter('convert_from_tags')('autorizzati'),
					tk.get_validator('ignore_missing')]
		})
